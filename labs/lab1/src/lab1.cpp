...
int main()
{
     ...
     * как были инициализированы поля класса.
     ...
     Поля класса не были инициализированы, в них лежит мусор.
     *
     ...
     * случае и в общем случае.
     ...
     Автоматически при создании обьекта вызывается конструктор для инициализации его полей
     После - вызвается деструктор, когда обьект выходит из области видимости, более не используется

     Если программист не указал ни одного конструктора, компилятор создает его
     автоматически. Такой конструктор вызывает конструкторы по умолчанию
     для полей класса и конструкторы по умолчанию базовых классов
     */

    {
        // TODO: а почему тогда тут такие странные числа, если были вызваны
        //       конструкторы по умолчанию для полей?

        /* 
        Поля rect имеют тип int. Собственного конструктора у этого класса пока нету, 
        поэтому для локального обьекта встроенные типы, в нашем случае инт, не инициализируются, в них мусор
        
        Неявный конструктор по умолчанию для полей , то есть встроенных типов, не сделал ничего, в полях лежит мусор.
        */
        Rect rect; // $1 = {top = -135812528, bottom = 32767, right = -8280, left = 32767}
        int y = 0;
    }
     ...
     * конструктора.
     ...
     Если обьявить конструктор в секции "приват", то он ничего не сможет сделать,
     конструкторы должны находится в секции паблик. 

     //TODO: вы уверены, что это относится к той ошибке, которая тут произвошла?
     //       перепроверьте, пожалуйста
     // Проморгал этот момент. Действительно, тут дело в другом - появился конструктор с параметрами 
     // То есть теперь у класса есть один явно прописанный конструктор, значит конструктор по умолчанию 
     // больше не создается автоматически, если мы хотим создать обьект без параметров, теперь нужно самостоятельно 
     // позаботиться о наличии соответствующего конструктора. Иначе - ошибка компиляции, компилятор не находит такого конструктора (без параметров)
    ...
    {
        Rect rect(1, 2, 3, 4); // явный конструктор 
        int y = 0;
    } 
    ...
    полей. Если класс содержит указатели или ссылки, это, скорее всего, будет
    неправильным, поскольку и копия, и оригинал будут указывать на одну и ту же
    область памяти.
     */

    {
        Rect rect1(1, 2, 3, 4);  // явный конструктор
        Rect rect2 = rect1;   // работает конструктор копирования
        Rect rect3(rect1); // опять конструктор копирования но в другом виде
    }
     ...
     * случае? Если деструктор явным образом не определен, компилятор автоматически создает
        пустой деструктор.
     ...
     * вызывается деструктор. 
     ...
     Деструктор в прошлых заданиях вызывается после выхода обьекта из области видимости,
     при завершении блока кода. 
     */
    ...
    {
        Rect r1; //  конструктор по умолчанию для r1
        Rect *pR = new Rect(1,2,1,2);   // конструктор с параметрами для динамического обьекта
        {
            Rect r2(r1); // конструктор копирования от r1 для r2
            Rect arRect[2]; // дважды конструктор по умолчанию для обьектор массива
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i) ; // за весь цикл единожды конструктор с параметрами, так как ключ слово статик
                Rect r4(*pR); // конструктор копирования от динамического обьекта для r4
                Rect r5(i,i,i,i); // конструктор с параметрами
            }  // при каждой итерации цикла вызываются деструкторы для r4 и r5
        } // деструкторы для r2 и массива обьектов
        delete pR;  // деструктор для динамческого обьекта
        int y = 0; 
    } // деструктор для r1

    // в самом конце программы деструктор для статик обьекта
    ...
    /**{
        Rect r1;
        std::cout << r1.top << std::endl; // приват-поля доступны только внутри методов самого класса
    } 
     ...
     * Напишите в комментариях, чем функция-член отличается от обычной функции.
     ...
     Функция-член находится внутри класса и имеет доступ ко всем его полям (приватным и нет), 
     просто функция имеет доступ только к публичным полям или методам класса, находится вне класса.
     Обычная функция не имеет this — все объекты передаются явно через параметры
     */
    ...
    {
        Rect r1(-2, 2, 3, 0), r2(3, 0, 3, 0);
        std::cout << r1.get_top() << "; " << r1.get_bottom() << '\n';
        std::cout << r1.get_left() << "; " << r1.get_right() << "\n \n";
        Rect r3 = bounding_rect(r1, r2); // вызвается 3 конструктора, так как передаются 2 копии в функцию и r3 новый
        std::cout << r3.get_top() << "; " << r3.get_bottom() << '\n';
        std::cout << r3.get_left() << "; " << r3.get_right() << "\n \n";
        print_rect(r3);
    } 
    ...
    {
        Rect r2(3, 0, 3, -2);
        std::cout << r2.get_height() << "; " << r2.get_width() << '\n';
        std::cout << r2.get_square() << '\n';
        r2.set_height(5);
        r2.set_width(-1);
        std::cout << r2.get_top() << "; " << r2.get_bottom() << '\n';
        std::cout << r2.get_left() << "; " << r2.get_right() << "\n \n";
    ...
    {
        Barrel alch(150, 100, 96); // бочка со спиртом
        Barrel water(150, 100, 0);
        int k = 0;

        while(alch.get_spirt() > 50.)
        {
            alch.pereliv(water, 1);
            water.pereliv(alch, 1);
            k++;
        }
        std::cout << k << " итераций"<< '\n'; // 46 при текущих условиях
    } 
    ...
    {
        MyString sroka("sth bout lab1");
        sroka.print(); 
        sroka.set(3, 'a');
        sroka.print(); 
        sroka.set_new_string("new str chaeck");
        sroka.print(); 
        sroka.read_line();
        sroka.print(); 
    }
     ...
     * было быть не так много.
     ...
     В функциях не пришлось ничего менять, основные изменения были в сеттере и конструкторе с параметром
     */

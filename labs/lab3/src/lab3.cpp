...
int main() {
    ...
    {
        Matrix m1(4), m2(4, 4, 1);
        Matrix m3 = (m1 + m2) * m1 * 4 / 2 - m2; // как и ожидалось : 
        /*
        3 1 1 1
        1 3 1 1
        1 1 3 1
     ...
     * Нельзя перегрузить методом оператор `*` для операндов `double` и `Matrix`.
     * так как метод вызывается у левого операнда
     * то есть для Matrix * double будет работать. но для double * Matrix, так как
     * такое действие требует перегрузки глобальной функцией.
     * 
     * 
     */
     ...
     * 
     * Эти операторы нельзя сделать методами класса MyString 
     * так как левый операнд имеет тип потока
     */
    ...
    {
        MyString s("123");
        MyString f = addTxtExtension(s);
        std::cout << "with extension: " << f << "\n";
     ...
     * 
     * итератор знает как найти каждый  доступный элемент в структуре данных 
     */
...

    {
        ...
        sths[0] = make_system(23, 15, "Hahahahaha system", Notification::syst_n::serios::NORM);
        sths[1] = make_message(23, 77, "yaroslav", "Priv kd chd?");
        sths[2] = make_app(15, 7, "BS", "new bp", "lets gooo");
        for (int i = 0; i < 3; i++) {
            sths[i].print();
        }
    }
     ...
     * 
     * Можно было бы создать абстрактный родительский класс Notification, полями которого
     * было бы время. Соответственно обьявить функцию принт чисто виртуальной, деструктор - виртуальеым.
     * создать три класса наследника, систем мэсэдж и апп соответственно. 
     * каждый из них бы имел нужные им поля, а также собственную реализацию принт.
     *  
     */
    ...
    {
        NotificationQueue ochered;

        ochered.push(make_system(25, 5, "sth has happend", Notification::syst_n::VAZHNO));
        ochered.push(make_message(10, 15, "Chel", "helllo"));
        ochered.push(make_app(11, 0, "sthh", "app number 1", "allright yooo !!!"));



        for (auto it = ochered.begin(); it != ochered.end(); ++it) {
            it->print();
        }

        std::cout << "Размер: " << ochered.size() << "\n";

        Notification n = ochered.pop();
        n.print();
    ...
    {
        BaseFile f1("test.bin", "wb");
        std::cout << "\nf1 открыт = " << f1.is_open() << "\n";

        BaseFile f2 = std::move(f1);
        std::cout << "сделали move. \n f1 открыт = " << f1.is_open() << "\n";
        std::cout << "f2 открыт = " << f2.is_open() << "\n";

        BaseFile f3;
        f3 = std::move(f2);
        std::cout << "сделали move \n f2 открыт = " << f2.is_open() << "\n";
        std::cout << "f3 открыт = " << f3.is_open() << "\n"; 
